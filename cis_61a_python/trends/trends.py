...
def find_centroid(polygon):
    ...
    >>> triangle = [p1, p3, p2, p1]  # First vertex is also the last vertex
    ...
    for i in range(len(polygon) - 1):
        ...
        area_total += cross_term


        lat_sum += (x1 + x2) * cross_term

        lon_sum += (y1 + y2) * cross_term

    signed_area = area_total / 2
    ...
    centroid_lat = lat_sum / (6 * signed_area)
    centroid_lon = lon_sum / (6 * signed_area)


    return (centroid_lat, centroid_lon, area)
...
def find_state_center(polygons):
    ...
    if total_area == 0:
        ...
        for polygon in polygons:
            ...
            centroid_lat, centroid_lon, third = find_centroid(polygon)

            lat_sum += centroid_lat

            lon_sum += centroid_lon

        return make_position(lat_sum / len(polygons), lon_sum / len(polygons))


    center_lat = lat_sum / total_area
    center_lon = lon_sum / total_area


    return make_position(center_lat, center_lon)
...
def group_tweets_by_state(tweets):
    ...
    for state_code, state_polygons in us_states.items():

        state_centers[state_code] = find_state_center(state_polygons)

    for tweet in tweets:
        tweet_locationa = tweet_location(tweet)
        ...
        for state_code, center in state_centers.items():
            distance = geo_distance(tweet_locationa, center)

            if distance < min_distance:
                min_distance = distance
                nearest_state = state_code

        # Add tweet to  state's list
        if nearest_state:
    ...
    for state, tweets in tweets_by_state.items():
        ...
        for tweet in tweets:
            ...
            if has_sentiment(tweet_sentiment):
                sentiment_sum += sentiment_value(tweet_sentiment) # <- type lint error only
                sentiment_count += 1

        if sentiment_count > 0:
            averaged_state_sentiments[state] = sentiment_sum / sentiment_count
